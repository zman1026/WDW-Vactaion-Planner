...
export default async function SharedTripPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  if (!trip) notFound();
  const parks = await prisma.parkEntity.findMany({ where: { id: { in: trip.dayPlans.flatMap((day) => day.parkId ? [day.parkId] : []) } }, select: { id: true, name: true } });
