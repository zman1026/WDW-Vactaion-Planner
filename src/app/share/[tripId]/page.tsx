...
export default async function SharedTripPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  const entityIds = [...trip.dayPlans.flatMap((day) => day.parkId ? [day.parkId] : []), ...(trip.hotelId ? [trip.hotelId] : [])];
    ...
    <ol className="space-y-7">{trip.dayPlans.map((day, index) => {
      ...
      return <li key={day.id} data-theme={theme.id} data-pattern={theme.pattern} className="day-theme share-day overflow-hidden rounded-card border border-border bg-surface shadow-card">
        <header className="day-theme__hero px-5 py-6 sm:px-7">
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold text-primary sm:text-3xl">{theme.displayName}</h2><p className="mt-1 text-sm text-muted">Day {index + 1} · {format(day.date, "EEEE, MMMM d")}</p></div><span className="rounded-full border border-[rgb(var(--day-accent)/.24)] bg-white/60 px-3 py-1 text-xs font-bold text-[rgb(var(--day-accent-deep))]">{day.items.length} plan{day.items.length === 1 ? "" : "s"}</span></div>
        </header>
        {day.items.length === 0 ? <p className="px-5 py-6 text-sm italic text-muted sm:px-7">{theme.emptyDescription}</p> : <ol className="px-5 py-3 sm:px-7">{day.items.map((item) => <li key={item.id} className="grid break-inside-avoid gap-1 border-t border-[rgb(var(--day-accent)/.16)] py-4 first:border-t-0 sm:grid-cols-[8rem_1fr] sm:gap-5">
          <span className="day-accent-text text-sm font-bold">{shareTiming(item)}</span>
          <div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-primary">{item.title}</p><span className="rounded-full border border-[rgb(var(--day-accent)/.2)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[rgb(var(--day-accent-deep))]">{itemLabel(item.entityType)}</span></div>{item.timingType === "EXACT" && item.endTime && <p className="mt-1 text-xs text-muted">Planned until {item.endTime}</p>}{item.notes && <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-muted">{item.notes}</p>}{item.estimatedCostCents !== null && <p className="mt-1 text-xs text-muted">Estimated ${(item.estimatedCostCents / 100).toFixed(2)}</p>}</div>
        </li>)}</ol>}
      </li>;
    })}</ol>
