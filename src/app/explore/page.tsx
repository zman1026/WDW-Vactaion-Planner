...
export default async function ExplorePage({ searchParams }: { searchParams: Promise<{ park?: string; type?: string; q?: string; live?: string }> }) {
  ...
  const trips = user ? await prisma.trip.findMany({ where: { userId: user.id }, orderBy: { startDate: "asc" }, select: { id: true, name: true, dayPlans: { orderBy: { date: "asc" }, select: { id: true, date: true } } } }) : [];
  const dayOptions = trips.flatMap((trip) => trip.dayPlans.map((day, index) => ({ id: day.id, label: `${trip.name} · Day ${index + 1} · ${format(day.date, "MMM d")}` })));
