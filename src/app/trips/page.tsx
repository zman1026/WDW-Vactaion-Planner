...
export default async function TripsPage() {
  ...
  return <div className="space-y-9">
    <header className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-gold">Your Disney vacations</p><h1 className="mt-2 text-4xl font-semibold text-primary sm:text-5xl">My trips</h1><p className="mt-2 max-w-2xl text-muted">Choose a trip and pick up right where you left off.</p></div><div className="flex flex-wrap gap-2"><Link href="/trips/new?path=guide" className={buttonStyles({ size: "lg" })}>Help me plan</Link><Link href="/trips/new?path=veteran" className={buttonStyles({ variant: "secondary", size: "lg" })}>I’ll plan it myself</Link></div></header>

    {summaries.length === 0 ? <EmptyState title="Your first adventure starts with a date" description="Answer a few easy questions for a ready-to-edit first draft, or choose open days if you already know the parks." href="/trips/new?path=guide" actionLabel="Help me plan my first trip" /> : <>
