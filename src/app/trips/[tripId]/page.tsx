...
interface TripPageProps { params: Promise<{ tripId: string }>; searchParams: Promise<{ day?: string; view?: string; edit?: string; new?: string; reservation?: string }> }
...
export default async function TripPage({ params, searchParams }: TripPageProps) {
  ...
  const view = requestedView ?? (query.day ? "day" : activeToday ? "today" : "overview");
  ...
  const reservationSummaries = trip.reservations.map((item) => {
    ...
    return { id: item.id, dayPlanId: item.dayPlanId ?? dayIdByDate.get(date) ?? null, category: item.category, title: item.title, date, startTime: item.startTime, endTime: item.endTime, status: item.status, confirmationNumber: item.confirmationNumber, location: item.location, notes: item.notes, costCents: item.costCents, partySize: item.partySize };
  });
  const itineraryBookings = trip.dayPlans.flatMap((day) => day.items.filter((item) => item.bookingStatus === "BOOKED" || item.bookingStatus === "WISHLIST").map((item) => ({ id: item.id, dayPlanId: day.id, title: item.title, entityType: item.entityType, date: format(day.date, "yyyy-MM-dd"), startTime: item.startTime, status: item.bookingStatus, confirmationNumber: item.confirmationNumber, estimatedCostCents: item.estimatedCostCents, partySize: item.partySizeOverride, notes: item.notes })));
  const progress = calculateTripProgress({ hotelId: trip.hotelId ?? trip.customHotelName, budgetCents: trip.budgetCents, hasPartyProfile: Boolean(trip.partyProfile), mustDoCount: trip.mustDos.length, days: trip.dayPlans, reservations: reservationSummaries.map((item) => ({ status: item.status, dayPlanId: item.dayPlanId })) });
  ...
  return <div className="planner-page space-y-4">
    ...
    {view === "overview" ? <TripOverview tripId={trip.id} hotelName={hotelName} partySize={partyProfile.partySize} companionCount={trip.companions.length} reservationCount={reservationSummaries.length + itineraryBookings.filter((item) => item.status === "BOOKED").length} openReservationCount={reservationSummaries.filter((item) => item.status !== "CONFIRMED").length + itineraryBookings.filter((item) => item.status === "WISHLIST").length} progress={progress} mustDos={trip.mustDos} plannedCostCents={plannedCostCents} budgetCents={trip.budgetCents} days={trip.dayPlans.map((day) => ({ id: day.id, date: day.date, parkName: day.parkId ? parkNames.get(day.parkId) ?? null : null, secondaryParkName: day.secondaryParkId ? parkNames.get(day.secondaryParkId) ?? null : null, notes: day.notes, reservationCount: reservationSummaries.filter((item) => item.dayPlanId === day.id).length, items: day.items.map((item) => ({ entityType: item.entityType, title: item.title, timingType: item.timingType, startTime: item.startTime, bookingStatus: item.bookingStatus })) }))} /> : view === "reservations" ? <ReservationCenter key={`${query.new ?? ""}-${query.reservation ?? ""}-${query.day ?? ""}`} tripId={trip.id} tripStartDate={format(trip.startDate, "yyyy-MM-dd")} days={days} reservations={reservationSummaries} itineraryBookings={itineraryBookings} initialDayId={query.day} initialReservationId={query.reservation} startNew={query.new === "1"} /> : view === "today" && todayDay ? <TodayView tripId={trip.id} activeToday={activeToday} hotelName={hotelName} companionCount={trip.companions.length} reservations={reservationSummaries} day={{ id: todayDay.id, date: format(todayDay.date, "yyyy-MM-dd"), parkName: todayDay.parkId ? parkNames.get(todayDay.parkId) ?? null : null, secondaryParkName: todayDay.secondaryParkId ? parkNames.get(todayDay.secondaryParkId) ?? null : null, notes: todayDay.notes, items: todayDay.items.map((item) => ({ id: item.id, title: item.title, entityType: item.entityType, timingType: item.timingType, timeOfDay: item.timeOfDay, startTime: item.startTime, endTime: item.endTime, bookingStatus: item.bookingStatus, confirmationNumber: item.confirmationNumber, notes: item.notes })) }} /> : <>
