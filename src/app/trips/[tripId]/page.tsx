...
export default async function TripPage({ params, searchParams }: TripPageProps) {
  ...
  const paidExtrasCents = allItems.filter((item) => item.paidExtraType).reduce((total, item) => total + (item.estimatedCostCents ?? 0), 0);
  ...
  return <div className="planner-page space-y-4">
    <header className="planner-masthead relative overflow-hidden rounded-[1.5rem] border border-primary/10 bg-primary p-4 text-white shadow-lift sm:p-5">
      ...
      <div className="relative flex flex-col gap-4 xl:flex-row xl:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2"><div className="min-w-56 flex-1 rounded-control border border-white/10 bg-white p-3 text-ink sm:flex-none"><BudgetMeter plannedCents={plannedCostCents} budgetCents={trip.budgetCents} paidExtrasCents={paidExtrasCents} compact /></div><TripParty tripId={trip.id} companions={trip.companions} /><TripSettings trip={{ id: trip.id, name: trip.name, startDate: format(trip.startDate, "yyyy-MM-dd"), endDate: format(trip.endDate, "yyyy-MM-dd"), budget: trip.budgetCents === null ? "" : (trip.budgetCents / 100).toFixed(2), notes: trip.notes ?? "", hotelId: trip.hotelId, customHotelName: trip.customHotelName, partyProfile }} hotels={hotels} directoryStatus={{ hotelCount: hotels.length, lastSynced: directoryStatus._max.lastSynced?.toISOString() ?? null }} /></div>
      </div>
    </header>
    ...
    <nav className="flex items-center gap-1 rounded-control border border-border bg-surface p-1 shadow-card" aria-label="Planner views">
      ...
      <Link href={`/trips/${trip.id}?view=today${todayDay ? `&day=${todayDay.id}` : ""}`} aria-current={view === "today" ? "page" : undefined} className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-sm font-bold transition sm:flex-none ${view === "today" ? "bg-primary text-white shadow-sm" : "text-muted hover:bg-parchment hover:text-primary"}`}><SparkleIcon />Today{!activeToday && <span className="hidden rounded-full bg-sand/30 px-1.5 py-0.5 text-[8px] uppercase text-gold lg:inline">Preview</span>}</Link>
      <span className="ml-auto hidden pr-3 text-xs text-muted md:block">{progress.label} · {progress.score}% complete</span>
    </nav>
