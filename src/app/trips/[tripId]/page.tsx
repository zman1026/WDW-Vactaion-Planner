          ...
          <ol className="flex min-w-max gap-2">
            {trip.dayPlans.map((day, index) => {
              ...
              return <li key={day.id}><Link href={`/trips/${trip.id}?day=${day.id}#day-canvas`} aria-current={active ? "date" : undefined} className={`block min-h-16 w-36 rounded-control border p-3 transition ${active ? "day-accent-border bg-white/80 shadow-card" : "border-transparent bg-white/45 hover:border-[rgb(var(--day-accent)/.2)] hover:bg-white/75"}`}>
                ...
                <span className="mt-1 block max-w-full truncate text-[11px] text-muted">{dayParkName || hotel?.name || "Resort day"}</span>
              </Link></li>;
            })}
          </ol>
        </div>
        ...
        <Card className="day-theme__hero mt-4 overflow-hidden p-0" key={selectedDay.id}>
          <header className="border-b border-[rgb(var(--day-accent)/.18)] px-5 py-7 sm:px-8 sm:py-9">
            ...
            <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
              <div><h2 className="max-w-3xl text-3xl font-semibold text-primary sm:text-4xl">{selectedTheme.displayName}</h2><p className="mt-1 font-display text-lg text-muted">{format(selectedDay.date, "EEEE, MMMM d, yyyy")}</p></div>
              <span className="rounded-full border border-[rgb(var(--day-accent)/.25)] bg-white/60 px-3 py-1 text-xs font-bold text-[rgb(var(--day-accent-deep))]">Day {selectedIndex + 1}</span>
            </div>
          </header>
          <div className="p-5 sm:p-8">
            <DayPlanner tripId={trip.id} dayPlanId={selectedDay.id} parkId={selectedDay.parkId} parks={parks} items={selectedDay.items} emptyTitle={selectedTheme.emptyTitle} emptyDescription={`${selectedTheme.emptyDescription}${specialDay === "Arrival day" ? " Consider check-in, your transfer, and an easy first meal." : specialDay === "Departure day" ? " Consider checkout, luggage, your transfer, and one last relaxed stop." : ""}`} days={trip.dayPlans.map((day, index) => ({ id: day.id, label: `Day ${index + 1} · ${format(day.date, "MMM d")}` }))} />
          </div>
        </Card>
      </section>}
    </div>
