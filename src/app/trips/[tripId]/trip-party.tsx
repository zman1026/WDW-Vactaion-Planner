...
export function TripParty({ tripId, companions }: { tripId: string; companions: CompanionSummary[] }) {
  ...
  return <><Button type="button" variant="secondary" size="sm" className="border-white/15 bg-white/10 text-white hover:bg-white/20" onClick={() => setOpen(true)}><PeopleIcon />Party{companions.length ? ` · ${companions.length + 1}` : ""}</Button><Modal open={open} title="Your travel party" onClose={() => setOpen(false)}><div className="space-y-5">
