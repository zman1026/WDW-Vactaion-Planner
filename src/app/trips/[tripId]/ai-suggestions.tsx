...
export function AiSuggestions({ tripId, dayPlanId, hasPark, disabled, onApply }: { tripId: string; dayPlanId: string; hasPark: boolean; disabled: boolean; onApply: (items: SuggestedItem[]) => void }) {
  ...
  return <section className="space-y-3 rounded-xl border border-purple-200 bg-purple-50/60 p-4">
    ...
    <textarea value={preferences} onChange={(event) => setPreferences(event.target.value)} maxLength={1000} rows={3} placeholder="Optional: add preferences just for this day. Your saved party profile is included automatically." className="w-full rounded-lg border bg-white px-3 py-2 text-sm" />
    <button type="button" disabled={loading} onClick={suggest} className="rounded-full bg-purple-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{loading ? "Building ideas…" : "Generate suggestions"}</button>
