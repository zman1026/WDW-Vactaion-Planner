...
export function TripProgressMeter({ progress, compact = false }: { progress: TripProgressValue; compact?: boolean }) {
  return <div className={clsx(!compact && "rounded-control border border-border bg-parchment/55 p-4")}>
    ...
    {!compact && <div className="mt-2 flex items-center justify-between gap-3 text-xs"><span className="font-semibold text-success">{progress.label}</span><span className="text-muted">{progress.completedSteps} of {progress.totalSteps} planning steps ready</span></div>}
  </div>;
}
